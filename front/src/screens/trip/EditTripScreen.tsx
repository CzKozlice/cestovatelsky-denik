...
export default function EditTripScreen() {
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      ...
      try {
        const res = await axios.get(`${API_URL}/auth/trips/${tripId}`, {
        ...
        setSelectedCountries(trip.countries.map((c: any) => c.country.id));
      } catch (error) {
  ...
  const handleUpdate = async () => {
    if (!name || !startDate || !endDate || selectedCountries.length === 0) {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      await axios.patch(`${API_URL}/auth/trips/${tripId}`, {
        name,
        startDate,
        endDate,
      ...
      console.error(error);
      alert('Chyba při aktualizaci výletu.');
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.container}>
          ...
          <Text style={styles.sectionTitle}>Upravit výlet</Text>
          ...
          <View style={styles.buttonRow}>
            ...
            <TouchableOpacity style={[styles.button, { backgroundColor: '#7e57c2' }]} onPress={handleUpdate}>
